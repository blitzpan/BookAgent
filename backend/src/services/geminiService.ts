// services/geminiService.ts（后端端口）
//
// 多模型改造后的"编排层"：保留原有全部导出函数签名，内部按角色（TEXT/IMAGE/VISION）
// 选择对应的 ModelBackend（见 ./providers）。默认全 gemini，行为与原实现 100% 一致。
//
// 与前端版本的差异：
//  - parseStoryIntoPages 入参由 File 改为磁盘图片路径（imagePath: string | null）；
//  - generateImage 增加可选的 aspectRatio / imageSize，便于生图 run 注入配置；
//  - 其余一致性逻辑（CHARACTER LOCK / 多参考图 / 三重 Director / FIX 指令）逐字保留。

import { Type } from "@google/genai";
import type { StoryPage } from "../types";
import { getTextBackend, getImageBackend, getVisionBackend } from "../providers";
import { pathToInlineImagePart, dataUrlToInlineImagePart } from "../providers/util";
...
Return ONLY JSON that matches the schema.`;
    ...
    const cleaned: ExtractedCharacter[] = chars
      ...
      .filter((c: any) => c.name.trim().length > 0 || c.visualDescription.trim().length > 0)
      .map((c: any) => ({
        ...c,
...
"${story}".`;
    ...
    if (imagePath) {
      const imagePart = pathToInlineImagePart(imagePath);
