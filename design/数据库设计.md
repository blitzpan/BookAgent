...
CREATE TABLE stories (
  ...
  status                    TEXT,                       -- 状态:新建/AI改写中/改写完成待生图/生图中/生图完成待审批/待审批发行/审批通过/删除
...
CREATE TABLE generation_runs (
  ...
  max_frame_retry     INTEGER,              -- 单页最多重画次数(默认3)
  sequence_threshold  REAL,                 -- 整书一致性分数线(默认0.8)
  max_sequence_retry  INTEGER,              -- 整书修复最多轮(默认1,默认不自动修)
  initial_retry_budget INTEGER,             -- 首跑每页生成张数(默认1,即"不浪费")
...
| `generation_runs.initial_retry_budget` | 首跑每页张数，默认 1（按需哲学核心参数） |
...
2. **锚图挂 run**：换 style 重跑，锚图随之更新，显示某故事的锚图 = 取该故事最新/默认 run 的 `characters`；
3. **默认/备用 = 同页多行**：`is_default` 区分；重画单页 = 插入新行并翻转 `is_default`；
4. **run 幂等**：worker 处理 run 时先查已有产物，跳过已完成页，支持宕机续跑；
