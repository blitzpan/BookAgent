...
2. 生图**按需**：首跑每页生成尝试次数由 `initial_retry_budget` 控制（默认 1，即每页只生 1 张并评分存储；配置更大值则在预算内按阈值循环、落选图存为备用 `kind=retry`）。首跑结束后**不自动补画**，如需更多候选由用户手动 `manual_new` 扩展，避免一次性海量生图。
...
- **worker 处理一个 run 时分阶段、且每阶段先查「是否已有产物」**（幂等按 `generation_run_id` 作用域，而非全局页默认）：
  1. 阶段一（仅整书 run）：本 run 的 `characters`（锚图）是否齐？不齐才生成；**单页 `manual_new` run 跳过此阶段，直接复用该 story 最近整书 run 的锚图**（锚图 run 概念）；
  2. 阶段二：逐页检查「本 run 是否已存在 `is_default=1` 的 `page_images`」，有就跳过，只补缺失页（按 run 作用域判定，避免换 style 整书重跑被误跳过）；
...
sequenceDiagram
    ...
    API->>DB: 建子任务(scope=single_page,target_page_id), 复用该页 prompts + 最近整书 run 的锚图
    API-->>前端: 202
    Note over WK: 异步生成新图(最多 maxFrameRetry 次) → 插 page_images(kind=manual_new, is_default=0 作备用), 由用户 PATCH 翻转默认
...
6. **run 幂等可重入**，支持宕机后续跑（按 `generation_run_id` 作用域判定已完成页）；
7. **状态迁移守卫**：所有写操作经 `canTransition(from,to)` 校验，非法迁移直接拒绝；
8. **静态资源路径穿越防护**：`/assets/*` 拒绝 `..` 与绝对路径，防止任意文件读取。
